...
func AddGeneric(c *gin.Context, db *gorm.DB) {
	...
	newGeneric := models.Generic{
		Name:      payload.GenericName,
		UpdatedBy: userObj.ID,
	}
	if err := db.Create(&newGeneric).Error; err != nil {
		logrus.WithError(err).WithField("generic_name", payload.GenericName).Error("Failed to add generic to database")
...
func DeleteGeneric(c *gin.Context, db *gorm.DB) {
	...
	id := c.Param("id")
	if err := db.Delete(&models.Generic{}, id).Error; err != nil {
