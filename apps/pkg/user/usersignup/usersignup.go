...
// UserSignUp handles signup OTP request
func UserSignUp(c *gin.Context, db *gorm.DB) {
	var req config.SignUpReq
	...
	if err := db.Where("email = ?", req.Email).First(&user).Error; err == nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "This email is already registered. Please sign in via email.",
		})
		return
	}

	// Check if phone already exists
	if err := db.Where("phone = ?", req.Phone).First(&user).Error; err == nil && user.PhoneVerified {
		c.JSON(http.StatusBadRequest, gin.H{
...
// SendOTP for generate and send OTP.
func sendOTP(c *gin.Context, db *gorm.DB, phone string) {
...
func VerifySignupOTP(c *gin.Context, db *gorm.DB) {
	...
	// Create user now
	user := models.User{
		FirstName:       verifyReq.FirstName,
		LastName:        verifyReq.LastName,
		Email:           &verifyReq.Email,
		Phone:           &verifyReq.Phone,
		PhoneVerified:   true,
		EmailVerified:   true,
...
func ResendOTP(c *gin.Context, db *gorm.DB) {
	var req config.ResendOTPReq
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid input"})
		return
	}
