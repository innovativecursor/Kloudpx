...
func (h *MedicineHandler) GetMedicine(c *gin.Context) {
	...
	var medicine models.Medicine
	if err := h.db.Preload("Generic").First(&medicine, medicineID).Error; err != nil {
	...
	// FIX: Use medicine.Generic.Name directly (it's a value, not pointer)
	genericName := ""
	if medicine.Generic.Name != "" {
...
func (h *MedicineHandler) GetMedicines(c *gin.Context) {
	...
	page, pageSize := getMedicinePaginationParams(c)
	offset := (page - 1) * pageSize

	// Initialize query
	query := h.db.Model(&models.Medicine{}).Preload("Generic")
...
func (h *MedicineHandler) SearchMedicines(c *gin.Context) {
	...
	// Prepare response
	response := struct {
		Results    []models.Medicine `json:"results"`
...
// Unique pagination helper for medicines
func getMedicinePaginationParams(c *gin.Context) (int, int) {
	page := 1
	if p := c.Query("page"); p != "" {
