...
func AdminLogin(c *gin.Context) {
	...
	token, err := auth.GenerateJWT(admin.ID, admin.Username, "admin")
	if err != nil {
...
func AddMedicine(c *gin.Context) {
	...
	err := database.DB.QueryRow(
		...
		medicine.Name, medicine.GenericName, medicine.PrescriptionRequired, medicine.Stock, medicine.Price,
	).Scan(&medicine.ID)
...
func UpdateMedicine(c *gin.Context) {
	...
	result, err := database.DB.Exec(
		`UPDATE medicines SET 
			...
			price = $5 
		WHERE id = $6`,
		medicine.Name, medicine.GenericName, medicine.PrescriptionRequired, 
		medicine.Stock, medicine.Price, medicine.ID,
	)
...
func ListMedicines(c *gin.Context) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&med.ID, &med.Name, &med.GenericName, &med.PrescriptionRequired, &med.Stock, &med.Price,
		); err == nil {
...
// GetMedicineDetails retrieves detailed information about a specific medicine
func GetMedicineDetails(c *gin.Context) {
	...
	err = database.DB.QueryRow(`
		...
		&medicine.PrescriptionRequired, &medicine.Stock, &medicine.Price,
	)
